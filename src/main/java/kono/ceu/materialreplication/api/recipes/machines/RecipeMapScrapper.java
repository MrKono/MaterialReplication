...
public class RecipeMapScrapper extends RecipeMap<SimpleRecipeBuilder> {
    public RecipeMapScrapper(String unlocalizedName, int maxInputs, int maxOutputs, int maxFluidInputs, int maxFluidOutputs, SimpleRecipeBuilder defaultRecipe, boolean isHidden) {
        ...
        for (ItemStack input : inputs) {
            if (input != null && input != ItemStack.EMPTY) {
                ...
                return recipeBuilder()
                        .input(GTRecipeItemInput.getOrCreate(inputStack, 1))
                        .output(SCRAP)
                        .duration(20).EUt(VA[LV]).build().getResult();
            }
        }
